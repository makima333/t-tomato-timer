...
export class TaskDBClient extends SqlBase {
	...
	async update(taskId: number, taskName: string): Promise<any> {
		return this.db.execute(`UPDATE ${TaskDBClient.TABLE_NAME} SET name = $1 WHERE id = $2`, [
			taskName,
			taskId
		]);
	}
	...
	async delete(taskId: number): Promise<any> {
		this.db.execute(`UPDATE ${TaskDBClient.TABLE_NAME} SET deleted = true WHERE id = $1`, [taskId]);
	}
}
